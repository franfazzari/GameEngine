...
// The text texture that specify recording device names
LTexture gDeviceTextures[MAX_RECORDING_DEVICES];

// Number of avaiblable devices
							...
							if (e.type == SDL_KEYDOWN) {
								...
								if (e.key.keysym.sym >= SDLK_0 && e.key.keysym.sym <= SDLK_9) {
									...
									if (index < gRecordingDeviceCount) {
										...
										// Ddevice failed to open
										if (recordingDeviceId == 0) {

											// Report rerror
											printf("Failed to open recording device! SDL Error: %s\n", SDL_GetError());
											gPromptTexture.loadFromRenderedText("Failed to oipen recording device!", gTextColor);
											currentState = ERROR;
										}
										// Device opened successfully
											...
											playbackDeviceId = SDL_OpenAudioDevice(NULL, SDL_FALSE, &desiredPlayabackSpec, &gReceivedPlaybackSpec, SDL_AUDIO_ALLOW_FORMAT_CHANGE);
											...
											// Device opened successfully
												...
												// Go on to next state
												gPromptTexture.loadFromRenderedText("Press 1 to record 5 seconds.", gTextColor);
